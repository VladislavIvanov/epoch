...
peer_keys() ->
    [{dev0, {<<120,30,108,92,13,32,45,162,66,181,135,13,102,186,226,7,134,64,127,57,44,122,62,198,148,18,128,51,162,218,180,97>>,
...
sign_keys() ->
    [{dev0, {<<238,121,108,68,47,65,15,139,26,172,250,135,122,63,231,52,188,121,206,144,200,39,37,112,172,29,216,205,172,56,241,4,217,202,108,173,192,99,
...
create_configs(CTConfig, CustomConfig, Options) ->
    [create_config(N, CTConfig, CustomConfig, Options) || N <- [dev0, dev1, dev2, dev3]].
...
make_multi(Config) ->
    make_multi(Config, [dev0, dev1, dev2, dev3]).
...
node_tuple(N) when N == dev0; N == dev1; N == dev2; N == dev3 ->
    {N, node_name(N)}.

node_name(N) when N == dev0; N == dev1; N == dev2; N == dev3 ->
...
config_apply_options(Node, Cfg, [{add_peers, true}| T]) ->
    Cfg1 = Cfg#{<<"peers">> =>
              [peer_info(N1) || N1 <- [dev0, dev1, dev2, dev3] -- [Node]]},
    config_apply_options(Node, Cfg1, T).
